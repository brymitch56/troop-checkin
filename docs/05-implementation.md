...
**iCal sync.** Nightly + on-demand, and ~15 s after the server starts. Events are discrete (no RRULE by design). The whole sync is ONE transaction, so anything that throws inside it undoes everything — a sync must never be able to die on its own bookkeeping (it has, twice; both are now tests).

*Which row is this feed event?* A member-portal UID is `<head>-<event id>-<tail>`, and the tail is a timestamp that **changes whenever the event is edited in the portal** — a moved start, but also edits that alter nothing the feed shows. So identity is the stable middle segment (`attendanceSync.tlcEventIdFromUid`), not the whole UID: an edited event UPDATES its existing row — `ical_uid` and `start_at` included — and keeps its id, its app-owned settings (form requirement, Warn/Block, adult tracking), its cached form statuses and its history. A pair that was split before this rule existed is folded back: the row carrying history or settings is kept and moved onto the feed's values, and a bare duplicate (nothing attached, nothing set by hand) is removed and counted as `merged`. Two limits, both deliberate: if a row already holds the feed's exact `(ical_uid, start_at)` and is not bare, IT is the keeper and nothing is merged (the schema has `UNIQUE (ical_uid, start_at)`; moving the identity onto another row would be a constraint error); and sign-ins are never merged or dropped automatically — when both rows have their own, the live one is current and the other stays flagged. UIDs with no portal id (manual events, foreign feeds), and a portal id that appears twice in one feed, keep the plain `(ical_uid, start_at)` match.

*What happens to an event that left the feed?* It is deleted unless something of record hangs off it. "Of record" is read from the schema, not listed: every table with a foreign key to `event(id)` counts as history — sign-ins, texts sent, attendance pushes, skipped-advancement records, and any table a later migration adds — EXCEPT `event_form_status`, a cache rebuilt from the portal, which is deleted along with the event. An event with history is kept and flagged `removed_from_feed`; returning to the feed clears the flag. The admin "delete event" route applies the same rule (`icalSync.deleteEventUnlessHistory`). The last successful run is in `meta.last_ical_sync` (`{at, added, updated, flagged, deleted, merged, feed_events}`); a FAILED run is only a log line (`ical sync failed: …`), so watch the age of `at`, not just its presence.
